...
# Bound enumeration time for unusually complex molecules.
try:
...
def strict_tautomer_view(smiles):
    """Count only canonical representation changes caused by standardization.

    Both input and standardized molecules are canonicalized before comparison,
    so an ordinary SMILES serialization change is not mislabeled as a tautomer
    change.
    """
    original = str(smiles)
    ...
    try:
        ...
        # A valid tautomer transformation preserves formula and heavy-atom count.
        ...
        # Preserve dataset-provided SMILES when the molecular representation is unchanged.
        if real_changed:
