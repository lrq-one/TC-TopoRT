...
class RingEdgeCellRefiner(nn.Module):
    ...
    def forward(self, edge_h, edge_aux):
        aux_h = self.aux_proj(edge_aux.float())
        z = torch.cat([edge_h, aux_h], dim=-1)
...
class TopoCellRTBlock(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch, edge_aux=None):
        ...
        if edge_aux is not None:
            edge_aux_half = edge_aux[:edge_aux.size(0) // 2, ...]
            edge_attr2 = self.ring_refine(edge_attr2, edge_aux_half)
...
class TopoCellRTNet(nn.Module):
    ...
    def forward(self, data, return_emb=False):
        ...
        edge_aux = edge_attr[:, -9:].clone()
        edge_index_reversed = edge_index[[1, 0], :]
        edge_index = torch.cat([edge_index, edge_index_reversed], dim=1)
        edge_attr = edge_attr.repeat(2, 1)
        edge_aux = edge_aux.repeat(2, 1)
        ...
        h1, edge_attr = self.conv1(h, edge_index, edge_attr, batch, edge_aux=edge_aux)
        h2, edge_attr = self.conv2(h1, edge_index, edge_attr, batch, edge_aux=edge_aux)
        h3, edge_attr = self.conv3(h2, edge_index, edge_attr, batch, edge_aux=edge_aux)
        h4, edge_attr = self.conv4(h3, edge_index, edge_attr, batch, edge_aux=edge_aux)
        h5, edge_attr = self.conv5(h4, edge_index, edge_attr, batch, edge_aux=edge_aux)
        h6, edge_attr = self.conv6(h5, edge_index, edge_attr, batch, edge_aux=edge_aux)
        ...
        out = self.out_lin(result).squeeze()

        if return_emb:
            z = self.contrast_proj(result)
