...
class Complex(object):
    ...
    """
    def __init__(self, *cochains: Cochain, y: torch.Tensor = None, 
                 dimension: int = None, global_feat: torch.Tensor = None):
        if len(cochains) == 0:
    ...
    def to(self, device, **kwargs):
        ...
        if self.global_feat is not None:
            self.global_feat = self.global_feat.to(device, **kwargs)
        return self
...
class ComplexBatch(Complex):
    ...
    def __init__(self,
        ...
        super(ComplexBatch, self).__init__(*cochains, y=y, global_feat=global_feat)
    ...
    def from_complex_list(cls, data_list: List[Complex], follow_batch=[], max_dim: int = 2):
        ...
        # Batch global features
        global_feat_list = [comp.global_feat for comp in data_list if comp.global_feat is not None]
        global_feat = None
        if len(global_feat_list) == len(data_list):
            global_feat = torch.stack(global_feat_list, dim=0)
            
        batch = cls(*batched_cochains, y=y, num_complexes=len(data_list), 
                    dimension=dimension, global_feat=global_feat)
