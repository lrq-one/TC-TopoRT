...
def make_l2sp_anchor(model):
    """
    L2-SP anchor for trainable transfer modules, excluding out_lin.
    out_lin is reset for z-score target scale, so it should remain free.
    """
    anchor = {}
    for name, p in model.named_parameters():
        if p.requires_grad and (not name.startswith("out_lin")):
...
def train_one_epoch(model, loader, optimizer, device, huber_beta, freeze_mode, y_mean, y_std, l2sp_anchor=None, l2sp_lambda=0.0):
    ...
    for batch in loader:
        ...
        if l2sp_anchor is not None and float(l2sp_lambda) > 0:
...
def main():
    ...
    ap.add_argument("--l2sp_lambda", type=float, default=0.0)
