...
def make_inner_split(tr_local, groups_all, seed, inner_val_frac):
    tr_local = np.asarray(tr_local, dtype=int)
    groups_tr = np.asarray(groups_all)[tr_local]

    n_unique = len(np.unique(groups_tr))
    if n_unique < 3:
        rng = np.random.default_rng(seed)
        perm = rng.permutation(len(tr_local))
        n_val = max(1, int(round(len(tr_local) * inner_val_frac)))
        inner_val_rel = perm[:n_val]
        inner_tr_rel = perm[n_val:]
        return tr_local[inner_tr_rel], tr_local[inner_val_rel]
    ...
    dummy_x = np.zeros(len(tr_local))
    dummy_y = np.zeros(len(tr_local))
    inner_tr_rel, inner_val_rel = next(splitter.split(dummy_x, dummy_y, groups_tr))
...
def run_one_view_cv(
    ...
    sub = meta[meta["dataset_name"] == dataset_name].copy().reset_index(drop=True)
    ...
    targets_all = meta.sort_values("stage4_index")["rt"].values
    wrapped = ExternalTargetDataset(base_dataset, targets=targets_all)
    ...
    if args.group_cv:
        ...
        print(f"[Outer GroupKFold] dataset={dataset_name} view={view_name} group_col={args.group_col} n_groups={n_groups} folds={k}")
    else:
        groups = sub["stage4_index"].astype(str).values
        ...
        print(f"[Outer KFold] dataset={dataset_name} view={view_name} rows={len(sub)} folds={k}")
    ...
    for cv_fold, (tr_local, te_local) in enumerate(split_iter):
        ...
        inner_tr_local, inner_val_local = make_inner_split(
            tr_local=tr_local,
            groups_all=groups,
            seed=seed,
        ...
        train_global = global_indices[inner_tr_local]
        val_global = global_indices[inner_val_local]
        test_global = global_indices[te_local]

        train_loader = build_loader(wrapped, train_global, args.batch_size, shuffle=True)
        val_loader = build_loader(wrapped, val_global, args.eval_batch_size, shuffle=False)
        ...
        best_train_mae_at_val = float("inf")
        best_epoch = -1
        best_state = None
        bad = 0
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if improved:
                ...
                best_train_mae_at_val = float(train_mae)
            ...
            if epoch == 1 or epoch % args.log_every == 0 or epoch == args.epochs:
                print(
                    ...
                    f"outer_fold={cv_fold}/{k} epoch={epoch:03d} "
                    f"train_mae={train_mae:.4f} val_mae={val_mae:.4f} "
                    f"best_val={best_val_mae:.4f} best_epoch={best_epoch} "
                    f"n_inner_train={len(inner_tr_local)} n_inner_val={len(inner_val_local)} n_test={len(te_local)}"
                )

            if args.patience > 0 and bad >= args.patience:
                print(
                    f"[EARLY] {dataset_name} {view_name} outer_fold={cv_fold} "
                    f"no val improvement for {bad} epochs, best_epoch={best_epoch}, best_val={best_val_mae:.4f}"
                )
                break
        ...
        y_val_final, p_val_final = predict(model, val_loader, device)
        vm = metrics(y_val_final, p_val_final)
        ...
        fold_rows.append({
            ...
            "best_train_mae_at_val": float(best_train_mae_at_val),
            "n_inner_train": int(len(inner_tr_local)),
            "n_inner_val": int(len(inner_val_local)),
            "n_outer_test": int(len(te_local)),
            **{f"val_{kk}": vv for kk, vv in vm.items()},
...
def main():
    ...
    ap.add_argument("--out_dir", default="paper_analysis_stage4F_inner_val_tl")
    ...
    ap.add_argument("--datasets", nargs="+", default=["LIFE_old_194"])
    ap.add_argument("--run_keys", nargs="+", default=["seed1"])
    ap.add_argument("--source_folds", nargs="+", type=int, default=[0])

    ap.add_argument("--freeze_mode", default="rt_head_full",
    ...
    ap.add_argument("--group_cv", type=int, default=1)
    ap.add_argument("--group_col", default="inchikey")

    ap.add_argument("--inner_val_frac", type=float, default=0.15)
    ...
    ap.add_argument("--use_cosine", type=int, default=0)
    ...
    print("=== Stage 4F external TL with inner-val early stopping ===")
    ...
    print("inner_val_frac:", args.inner_val_frac)
        ...
        for run_key in args.run_keys:
            ...
            for source_fold in args.source_folds:
                ...
                keep_cols = [
                    ...
                    "origin_smiles", "taut_smiles", "rt", "formula",
                    "inchikey", "canonical_smiles", "taut_changed", "smrt_exact_overlap",
                    "run_key", "run_dir", "source_fold", "freeze_mode",
                ]
                keep_cols = [c for c in keep_cols if c in pred_origin.columns]

                merged = pred_origin[keep_cols + ["tl_pred", "tl_abs_error"]].rename(
                    columns={"tl_pred": "origin_tl_pred", "tl_abs_error": "origin_tl_abs_error"}
                )

                taut_keep = pred_taut[["stage4_index", "tl_pred", "tl_abs_error"]].rename(
                    columns={"tl_pred": "taut_tl_pred", "tl_abs_error": "taut_tl_abs_error"}
                )

                merged = merged.merge(taut_keep, on="stage4_index", how="left")
                merged["mean_tl_pred"] = 0.5 * (merged["origin_tl_pred"] + merged["taut_tl_pred"])
                merged["mean_tl_abs_error"] = np.abs(merged["rt"].values - merged["mean_tl_pred"].values)

                all_pred_rows.append(merged)
                all_fold_rows.append(fold_origin)
                all_fold_rows.append(fold_taut)

                metric_df = summarize_predictions(merged)
                print("\n[SUMMARY current]")
                print(metric_df[["dataset_name", "run_key", "source_fold", "method", "mae", "rmse", "r2", "spearman", "bias"]].to_string(index=False))
    ...
    fold_all = pd.concat(all_fold_rows, axis=0, ignore_index=True)
    metric_by_run = summarize_predictions(pred_all)
    summary = summarize_across_runs(metric_by_run)

    save_csv(pred_all, out_dir / "external_tl_predictions.csv")
    save_csv(fold_all, out_dir / "external_tl_fold_metrics.csv")
    save_csv(metric_by_run, out_dir / "external_tl_metrics_by_run.csv")
    ...
    cols = [
        ...
        "spearman_mean", "pearson_mean", "bias_mean",
    ]
    cols = [c for c in cols if c in summary.columns]
    print(summary[cols].sort_values("mae_mean").to_string(index=False))
