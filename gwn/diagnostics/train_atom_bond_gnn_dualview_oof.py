...
def main():
    ...
    parser.add_argument("--out_dir", default="gwn/results_AtomBondGNN_DualView_seed5")
    parser.add_argument("--cache_dir", default="gwn/cache_atom_bond_gnn")
    ...
    assert len(origin_train) == len(taut_train), "origin/taut train row mismatch"
    assert len(origin_test) == len(taut_test), "origin/taut test row mismatch"

    print(f"train N={len(origin_train)} test N={len(origin_test)}")
