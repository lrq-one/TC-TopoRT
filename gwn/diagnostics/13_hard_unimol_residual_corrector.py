...
def main():
    ...
    for a in np.linspace(0, 1.0, 101):
        pv = pva + a * rva
        m = metrics(yva, pv)
        rows.append((a, m["MAE"], m["P95"], m[">100"], m[">200"]))

    scan = pd.DataFrame(rows, columns=["alpha", "val_MAE", "val_P95", "val_100", "val_200"])
    best = scan.sort_values(["val_MAE", "val_P95"]).iloc[0]
    alpha = float(best["alpha"])

    print("\nselected alpha:", alpha)
    print("VAL hard base:", metrics(yva, pva))
    print("VAL hard corr:", metrics(yva, pva + alpha * rva))

    # 只修正有 Uni-Mol 特征的 test hard 分子，其他 test 分子保持原预测
    final = test.copy()
    pred_map = dict(zip(te["SMILES"], pte + alpha * rte))
    ...
    print("\nTEST hard corr:", metrics(yte, pte + alpha * rte))
