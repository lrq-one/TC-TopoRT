...
def train_one_epoch(model, loader, optimizer, device, huber_beta, freeze_mode, y_mean, y_std):
    ...
    for batch in loader:
        ...
        target_log = torch.log1p(torch.clamp(target_raw, min=0.0))
        target_z = (target_log - y_mean) / y_std
        ...
        pred_log = pred_z.detach() * y_std + y_mean
...
def predict(model, loader, device, y_mean, y_std):
    ...
    for batch in loader:
        ...
        pred_log = pred_z.view(-1) * y_std + y_mean
        pred_raw = torch.expm1(pred_log)
...
def run_one_view_cv(
    ...
    for cv_fold, (tr_local, te_local) in enumerate(split_iter):
        ...
        y_mean = float(np.mean(y_train_log))
        y_std = float(np.std(y_train_log))
        if y_std < 1e-6:
            y_std = 1.0
