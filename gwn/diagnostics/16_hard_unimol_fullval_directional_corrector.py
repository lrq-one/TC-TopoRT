...
def make_model_features(x, df):
    """
    Uni-Mol2 0/1/2 token feature + 原模型预测值。
    base_pred 加进去很重要，因为修正方向和 RT 区间强相关。
    """
...
def main():
    ...
    ap.add_argument("--out_csv", default="results_TopoCellRT_CWNReplace_orig/hard_unimol_directional_corrected_test.csv")
    args = ap.parse_args()
    ...
    xtr = make_model_features(xtr_raw, tr)
    xva = make_model_features(xva_raw, va)
    xte = make_model_features(xte_raw, te)
    ...
    # residual magnitude 用 Ridge 绝对值，方向用 classifier
    mag_va = np.abs(rva)
    mag_te = np.abs(rte)

    # full-val-aware selection:
    # 参数选择不能只看 hard val，否则会牺牲 full distribution 的 P95 / >100。
    base_val_hard_m = metrics(yva, pva)
    base_val_full_m = metrics(val["y"].values, val["base_pred"].values)

    rows = []

    for cap in [20, 30, 40, 50, 60, 80, 100, 120]:
        for alpha in np.linspace(0.0, 0.8, 81):
            for tau in [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80]:
                for min_mag in [0, 20, 40, 60, 80]:
                    use = (conf_va >= tau) & (mag_va >= min_mag)

                    corr = alpha * np.clip(mag_va, 0, cap) * sign_va

                    # hard val metric
                    pv_hard = pva.copy()
                    pv_hard[use] = pv_hard[use] + corr[use]
                    mh = metrics(yva, pv_hard)

                    # full val metric: 只有有 Uni-Mol 特征且 use=True 的分子被修正
                    final_val = val.copy()
                    pred_map_val = dict(zip(va.loc[use, "SMILES"], pv_hard[use]))
                    final_val["final_pred"] = final_val.apply(
                        lambda row: pred_map_val.get(row["SMILES"], row["base_pred"]),
                        axis=1,
                    )
                    mf = metrics(final_val["y"].values, final_val["final_pred"].values)

                    # score 主要看 full-val MAE，同时强惩罚 full-val >100/P95 变差
    ...
    print("VAL full base:", base_val_full_m)
    print("VAL hard base:", base_val_hard_m)

    use_va = (conf_va >= tau) & (mag_va >= min_mag)
    corr_va = alpha * np.clip(mag_va, 0, cap) * sign_va
    pva_corr = pva.copy()
    pva_corr[use_va] = pva_corr[use_va] + corr_va[use_va]

    print("VAL hard corr:", metrics(yva, pva_corr))
    print("\nTop val scan:")
    print(scan.sort_values(["score", "full_val_MAE"]).head(10).to_string(index=False))

    use_te = (conf_te >= tau) & (mag_te >= min_mag)
    corr_te = alpha * np.clip(mag_te, 0, cap) * sign_te
    pte_corr = pte.copy()
    pte_corr[use_te] = pte_corr[use_te] + corr_te[use_te]

    final = test.copy()
    pred_map = dict(zip(te.loc[use_te, "SMILES"], pte_corr[use_te]))
