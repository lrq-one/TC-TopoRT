...
def train_one_epoch(model, loader, optimizer, device, huber_beta, freeze_mode, y_mean, y_std, rt_tail_weight=0.0, rt_tail_power=1.0, rt_tail_clip=2.5):
    ...
    for batch in loader:
        ...
        per_sample_loss = F.smooth_l1_loss(pred_z, target_z, beta=huber_beta, reduction="none")

        # RT-tail weighted loss:
        # Eawag shows prediction compression:
        # low RT is over-predicted and high RT is under-predicted.
        # Upweight samples far from the fold mean in z-score space.
        tail_strength = float(rt_tail_weight)
        tail_power = float(rt_tail_power)
        tail_clip = float(rt_tail_clip)

        if tail_strength > 0:
            z_abs = torch.abs(target_z).clamp(max=tail_clip)
            sample_weight = 1.0 + tail_strength * torch.pow(z_abs, tail_power)
            loss = torch.sum(per_sample_loss * sample_weight) / torch.sum(sample_weight)
        else:
    ...
    for cv_fold, (tr_local, te_local) in enumerate(split_iter):
        ...
        for epoch in range(1, args.epochs + 1):
            train_loss, train_mae = train_one_epoch(
                ...
                y_mean, y_std,
                rt_tail_weight=args.rt_tail_weight,
                rt_tail_power=args.rt_tail_power,
                rt_tail_clip=args.rt_tail_clip,
            )
...
def main():
    ...
    ap.add_argument("--rt_tail_weight", type=float, default=0.0)
    ap.add_argument("--rt_tail_power", type=float, default=1.0)
    ap.add_argument("--rt_tail_clip", type=float, default=2.5)
