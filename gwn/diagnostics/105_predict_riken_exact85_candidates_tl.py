...
SPLIT_DIR = Path("experiments_candidate_filtering/riken_tl_exact85")
TRAIN_DIR = Path("experiments_candidate_filtering/riken_tl_exact85_training/seed42")
OUT_DIR = Path("experiments_candidate_filtering/riken_exact85_predictions_tl_seed42")
OUT_DIR.mkdir(parents=True, exist_ok=True)

CAND_VALID = SPLIT_DIR / "riken_test85_exactS11_candidates_valid.csv"
...
def main():
    ...
    meta = pd.read_csv(SPLIT_DIR / "riken_test85_exactS11_metadata.csv", dtype=str, low_memory=False).fillna("")
    meta = meta.reset_index(drop=True)
    meta["s11_row"] = np.arange(len(meta))
    qid_to_row = dict(zip(meta["query_id"].astype(str), meta["s11_row"]))

    cand = pd.read_csv(CAND_VALID, dtype=str, low_memory=False).fillna("")
    cand = cand[cand["query_id"].astype(str).isin(set(meta["query_id"].astype(str)))].copy()

    cand["s11_row"] = cand["query_id"].astype(str).map(qid_to_row).astype(int)
    ...
    cand["rt_sec"] = pd.to_numeric(cand["query_rt_sec"], errors="coerce")
    ...
    cand = cand.sort_values(
        ["s11_row", "candidate_rank", "candidate_score"],
        ascending=[True, True, False],
    ).reset_index(drop=True)

    unique = cand[["candidate_smiles_canon"]].drop_duplicates().reset_index(drop=True)
    unique["candidate_uid"] = ["riken_cand_%06d" % i for i in range(len(unique))]
    ...
    origin_csv = OUT_DIR / "riken_exact85_unique_candidate_origin_for_prediction.csv"
    taut_csv = OUT_DIR / "riken_exact85_unique_candidate_taut_for_prediction.csv"
    ...
    unique.to_csv(OUT_DIR / "riken_exact85_unique_candidate_smiles.csv", index=False)
    cand.to_csv(OUT_DIR / "riken_exact85_candidate_row_map.csv", index=False)

    print("=" * 100)
    print("[RIKEN candidate input]")
    print("exact85 queries:", len(meta))
    print("candidate rows before canon:", before_rows)
    ...
    unique_out.to_csv(OUT_DIR / "riken_exact85_unique_candidate_predictions_tl_seed42.csv", index=False)
    final.to_csv(OUT_DIR / "riken_exact85_candidate_predictions_tl_seed42.csv", index=False)
    ...
    qstat.to_csv(OUT_DIR / "riken_exact85_query_candidate_stats.csv", index=False)

    print("=" * 100)
    print("[saved]")
    print(OUT_DIR / "riken_exact85_candidate_predictions_tl_seed42.csv")
    print(OUT_DIR / "riken_exact85_unique_candidate_predictions_tl_seed42.csv")
    print(OUT_DIR / "riken_exact85_query_candidate_stats.csv")
    print("=" * 100)
