...
def load_valid_smiles(csv_path):
    ...
    for source_idx, row in df.iterrows():
        ...
        valid_rt.append(rt)
        valid_source_idx.append(int(source_idx))

    return valid_smiles, valid_rt, valid_source_idx


@torch.no_grad()
def export_predictions(model, loader, source_indices, smiles_list, save_path, device):
    model.eval()
    ...
    n = min(len(source_indices), len(smiles_list), len(preds), len(targets))

    out = pd.DataFrame({
        "Source_Index": source_indices[:n],
...
def main():
    ...
    train_smiles_all, _, train_source_idx_all = load_valid_smiles(args.train_csv)
    test_smiles_all, _, test_source_idx_all = load_valid_smiles(args.test_csv)
    ...
    train_smiles = [train_smiles_all[i] for i in train_indices]
    val_smiles = [train_smiles_all[i] for i in val_indices]

    train_source_idx = [train_source_idx_all[i] for i in train_indices]
    val_source_idx = [train_source_idx_all[i] for i in val_indices]

    test_smiles = test_smiles_all
    test_source_idx = test_source_idx_all
    ...
    export_predictions(
        ...
        train_source_idx,
        train_smiles,
        os.path.join(args.out_dir, "base_train_predictions_idx.csv"),
        device,
    )
    ...
    export_predictions(
        ...
        val_source_idx,
        val_smiles,
        os.path.join(args.out_dir, "base_val_predictions_idx.csv"),
        device,
    )
    ...
    export_predictions(
        ...
        test_source_idx,
        test_smiles,
        os.path.join(args.out_dir, "base_test_predictions_idx.csv"),
        device,
    )
