...
def attach_rt_adapter(model, adapter_dim=64, adapter_scale=0.2, adapter_dropout=0.05, device=None):
    """
    Low-rank RT adapter:
    Keep the pretrained TCDV representation/readout mostly intact.
    Insert a small residual adapter before out_lin.

    This is meant for medium external datasets where rt_head_full overfits
    and post-hoc calibration is too weak.
    """
    hidden = int(model.emb_dim * 2)

    model.rt_adapter = nn.Sequential(
        nn.LayerNorm(hidden),
        nn.Linear(hidden, int(adapter_dim)),
        nn.GELU(),
        nn.Dropout(float(adapter_dropout)),
        nn.Linear(int(adapter_dim), hidden),
    )

    # zero-init: initial function is exactly the original model
    nn.init.zeros_(model.rt_adapter[-1].weight)
    nn.init.zeros_(model.rt_adapter[-1].bias)

    model.rt_adapter_scale = float(adapter_scale)

    if device is not None:
        model.rt_adapter.to(device)

    def forward_with_adapter(self, data, include_partial=False):
        tokens, mask = self.cwn_adapter(data)

        tokens = self.trans_graph(tokens)
        add_x = tokens.sum(dim=1)
        ...
        if hasattr(self, "rt_adapter") and self.rt_adapter is not None:
            result = result + float(getattr(self, "rt_adapter_scale", 0.2)) * self.rt_adapter(result)
...
def set_trainable(model, freeze_mode):
    ...
    if freeze_mode == "rt_adapter_outlin":
        unfreeze_module(model.rt_adapter, "rt_adapter")
        unfreeze_module(model.out_lin, "out_lin")

    elif freeze_mode == "out_lin_only":
        unfreeze_module(model.out_lin, "out_lin")
...
def run_one_view_cv(
    ...
    for cv_fold, (tr_local, te_local) in enumerate(split_iter):
        ...
        if args.freeze_mode == "rt_adapter_outlin":
            attach_rt_adapter(
                model,
...
def main():
    ...
    ap.add_argument("--freeze_mode", default="out_lin_only",
                    choices=["rt_adapter_outlin", "out_lin_only", "head_plus_trans_out", "rt_head_full", "last_blocks", "all"])
    ...
    ap.add_argument("--adapter_scale", type=float, default=0.2)
    ap.add_argument("--adapter_dropout", type=float, default=0.05)
