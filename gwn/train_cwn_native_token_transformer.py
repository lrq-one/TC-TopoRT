...
from net.cwn_native_token_transformer import OGBEmbedCINppWithTokenTransformer
...
class Config:
    ...
    RESULT_DIR = os.getenv('RESULT_DIR', 'results_CWN_Native_TokenTransformer') 
...
def main():
    ...
    # --- 下方的模型初始化与训练循环保持原样不变 ---
    model = OGBEmbedCINppWithTokenTransformer(
        out_size=1,
        num_layers=6,
        hidden=256,
        dropout_rate=0.0,
        ...
        readout='sum',
        global_feat_dim=0,
        token_d_model=int(os.getenv('TOKEN_DMODEL', '256')),
        token_heads=int(os.getenv('TOKEN_HEADS', '4')),
        token_layers=int(os.getenv('TOKEN_LAYERS', '1')),
        token_dropout=float(os.getenv('TOKEN_DROPOUT', '0.05')),
        residual_init=float(os.getenv('TOKEN_RES_INIT', '0.0')),
    ).to(Config.DEVICE)
