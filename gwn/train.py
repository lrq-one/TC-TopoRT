...
class Config:
    # 🌟 1. 独立指定官方训练集和测试集的路径
    TRAIN_CSV_PATH = 'data/SMRT_train.csv' 
    TEST_CSV_PATH = 'data/SMRT_test.csv'
    
    # 🌟 2. 使用两个不同的文件夹存特征缓存，彻底防止图数据冲突
    TRAIN_DATA_ROOT = 'smrt_cwn_data_train'
    TEST_DATA_ROOT = 'smrt_cwn_data_test'
    
    RESULT_DIR = 'results_TopoRT_Final7' 
...
def main():
    ...
    # --- 下方的模型初始化与训练循环保持原样不变 ---
    model = OGBEmbedCINpp(out_size=1, num_layers=6, hidden=256, dropout_rate=0.0,use_coboundaries=True,jump_mode='cat',embed_edge=True,readout='sum',global_feat_dim=0).to(Config.DEVICE)
