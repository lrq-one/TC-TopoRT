...
analyses that require separately supplied inputs. The archive DOI will be
inserted after deposition.
...
MAEs, and thresholds. The same underlying query identifiers and experimental RT
labels are used across predictors within each dataset. Final evaluation queries
do not contribute to calibration.

The development queries were excluded from the reduced training pool used to
fit the calibration model. Final candidate predictions were generated by the
corresponding target-domain predictors trained on the larger parent training
pools, which included the development compounds. Thus, the development set was
held out with respect to calibration-model fitting, rather than from the final
predictor itself.

The script applies only `T_m = 3 × MAE_dev,m`, retains missing predictions, and
otherwise retains `abs_rt_delta <= T_m`. It never changes original MS-FINDER
ordering. Candidate reduction uses candidate records as denominator; retention
and Top-k use queries as denominator.
